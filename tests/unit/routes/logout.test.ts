import { describe, it, expect, beforeEach } from 'vitest';
import { createApp } from '../../../src/router';
import { D1Mock } from '../helpers/d1-mock';
import { KVMock } from '../helpers/kv-mock';
import type { Env } from '../../../src/types';
import { putSession, getSession } from '../../../src/kv/store';

const TEST_PRIVATE_KEY_PEM = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

const app = createApp();
let kvMock: KVMock;
let testEnv: Env;

const SESSION_ID = 'test-session-abc';

beforeEach(async () => {
  const d1 = new D1Mock();
  d1.createTable('users');
  d1.createTable('clients');
  kvMock = new KVMock();

  testEnv = {
    OIDC_DB: d1.asD1(),
    OIDC_KV: kvMock.asKV(),
    SIGNING_KEY_PRIVATE: TEST_PRIVATE_KEY_PEM,
    ADMIN_API_KEY: 'test-admin-secret',
    ISSUER: 'https://idp.example.com',
  };

  await putSession(testEnv.OIDC_KV, SESSION_ID, {
    userId: 'u1',
    email: 'a@example.com',
    createdAt: '2026-01-01T00:00:00Z',
  });
});

const logout = (params: Record<string, string> = {}, cookie = ''): Promise<Response> => {
  const url = new URL('http://localhost/logout');
  Object.entries(params).forEach(([k, v]) => url.searchParams.set(k, v));
  return app.fetch(
    new Request(url.toString(), {
      headers: cookie ? { cookie } : {},
    }),
    testEnv,
  );
};

describe('GET /logout', () => {
  it('clears the session cookie', async () => {
    const res = await logout({}, `sid=${SESSION_ID}`);
    const cookie = res.headers.get('set-cookie')!;
    expect(cookie).toContain('sid=;');
    expect(cookie).toContain('Max-Age=0');
  });

  it('deletes the session from KV', async () => {
    await logout({}, `sid=${SESSION_ID}`);
    const session = await getSession(testEnv.OIDC_KV, SESSION_ID);
    expect(session).toBeNull();
  });

  it('redirects to post_logout_redirect_uri when provided', async () => {
    const res = await logout(
      { post_logout_redirect_uri: 'https://app.example.com/signed-out' },
      `sid=${SESSION_ID}`,
    );
    expect(res.status).toBe(302);
    expect(res.headers.get('location')).toBe('https://app.example.com/signed-out');
  });

  it('renders confirmation page when no redirect URI provided', async () => {
    const res = await logout({}, `sid=${SESSION_ID}`);
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).toContain('signed out');
  });

  it('works gracefully with no session cookie', async () => {
    const res = await logout();
    expect(res.status).toBeLessThan(500);
  });
});
