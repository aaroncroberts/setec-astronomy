import { describe, it, expect, beforeEach } from 'vitest';
import { createApp } from '../../../src/router';
import { D1Mock } from '../helpers/d1-mock';
import { KVMock } from '../helpers/kv-mock';
import type { Env } from '../../../src/types';

const TEST_PRIVATE_KEY_PEM = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

const app = createApp();
let testEnv: Env;

beforeEach(() => {
  const d1 = new D1Mock();
  d1.createTable('users');
  d1.createTable('clients');

  testEnv = {
    OIDC_DB: d1.asD1(),
    OIDC_KV: new KVMock().asKV(),
    SIGNING_KEY_PRIVATE: TEST_PRIVATE_KEY_PEM,
    ADMIN_API_KEY: 'test-admin-secret',
    ISSUER: 'https://idp.example.com',
  };
});

const getLogin = (params: Record<string, string> = {}): Promise<Response> => {
  const url = new URL('http://localhost/login');
  Object.entries(params).forEach(([k, v]) => url.searchParams.set(k, v));
  return app.fetch(new Request(url.toString()), testEnv);
};

describe('GET /login', () => {
  it('returns 200 with Content-Type text/html', async () => {
    const res = await getLogin({ state: 'abc' });
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toContain('text/html');
  });

  it('contains email and password input fields', async () => {
    const res = await getLogin({ state: 'abc' });
    const html = await res.text();
    expect(html).toContain('type="email"');
    expect(html).toContain('type="password"');
  });

  it('includes state as a hidden field', async () => {
    const res = await getLogin({ state: 'my-state-token' });
    const html = await res.text();
    expect(html).toContain('name="state"');
    expect(html).toContain('value="my-state-token"');
  });

  it('shows error message when ?error= param is present', async () => {
    const res = await getLogin({ state: 'abc', error: 'Invalid credentials' });
    const html = await res.text();
    expect(html).toContain('Invalid credentials');
    expect(html).toContain('role="alert"');
  });

  it('does not show error block when no error param', async () => {
    const res = await getLogin({ state: 'abc' });
    const html = await res.text();
    expect(html).not.toContain('role="alert"');
  });

  it('escapes XSS in error message', async () => {
    const res = await getLogin({ state: 'abc', error: '<script>alert(1)</script>' });
    const html = await res.text();
    expect(html).not.toContain('<script>');
    expect(html).toContain('&lt;script&gt;');
  });

  it('escapes XSS in state value', async () => {
    const res = await getLogin({ state: '"><script>alert(1)</script>' });
    const html = await res.text();
    expect(html).not.toContain('<script>');
  });

  it('has no external resource references (no CDN)', async () => {
    const res = await getLogin({ state: 'abc' });
    const html = await res.text();
    // Should not reference any external URLs in src= or href=
    expect(html).not.toMatch(/https?:\/\/(?!idp\.example\.com)/);
  });

  it('form posts to /login', async () => {
    const res = await getLogin({ state: 'abc' });
    const html = await res.text();
    expect(html).toContain('action="/login"');
    expect(html).toContain('method="POST"');
  });
});
